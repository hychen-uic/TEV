...
#' @param delta control the amount of input from the supplementary data
#'
...
#'
#' @export
RVsdb=function(y,x,xsup=NULL,lam=0.2,niter=1, delta=1,
              alpha=c(0.05),KV=rep(0,3),know="no",nrep=1000){
  ...
  #x=as.matrix(x)
  #X=as.matrix(X)
  M=x%*%chol2inv(chol(t(x)%*%x+delta*t(xsup)%*%xsup))%*%t(x)*(n+N*delta)/p
